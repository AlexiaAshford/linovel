...
class Response:
    ...
    def get_catalogue_info_by_book_id(book_id: str):
        return src.http_utils.get(api_url=Vars.current_book_rul_rule.get("catalogue_info").format(book_id))

    @staticmethod
    def get_book_info_by_keyword(method: str, params: dict):
        response = src.http_utils.get(method=method, params=params,
                                      api_url=Vars.current_book_rul_rule.get("search_info"))
        return list(zip(
